...
Доступ ограничивается RBAC и подразделениями. Пользователь видит объекты закреплённых за ним подразделений; администратор видит всю систему.
...
Для защищённых методов выполните `POST /api/login`, скопируйте `access_token`, нажмите **Authorize** в Swagger и вставьте токен.
...
`user` — базовая роль зарегистрированного пользователя; четыре рабочие роли используются в основном интерфейсе.

| Возможность | Пользователь | Диспетчер | Техник | Руководитель | Администратор |
| --- | :---: | :---: | :---: | :---: | :---: |
